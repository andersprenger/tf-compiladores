...
%token WHILE, DO, FOR, BREAK, CONTINUE, TRUE, FALSE, IF, ELSE
...
opt_exp_stmt: exp { System.out.println("\tPOPL %EAX"); }
            |
            ;
...
cmd :  exp ';' { 
    ...
    | WHILE {
		int rotContinue = proxRot;
		int rotBreak = proxRot+1;
		continueLabels.push(rotContinue);
		breakLabels.push(rotBreak);
		proxRot += 2;
	...
	| BREAK ';' {
		if (breakLabels.empty()) yyerror("break fora de um laco");
		else System.out.printf("\tJMP rot_%02d\n", breakLabels.peek());
	}
	| CONTINUE ';' {
		if (continueLabels.empty()) yyerror("continue fora de um laco");
		else System.out.printf("\tJMP rot_%02d\n", continueLabels.peek());
	}
     ;
...
%%
  ...
  private Stack<Integer> continueLabels = new Stack<Integer>();
  private Stack<Integer> breakLabels = new Stack<Integer>();
